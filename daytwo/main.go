...
func main() {
	...
	for _, line := range lines {
		...
		gameResults := strings.TrimLeft(splitByColon[1], " ")
		gameNameWords := strings.Split(gameName, " ")
		gameIndex, err  := strconv.Atoi(gameNameWords[1])
		if err != nil {
			fmt.Printf("%w", err)
			return
		}
		...
		isImpossible := false
		gameRounds := strings.Split(gameResults, "; ")
		for _, round := range gameRounds {
			pulls := strings.Split(round, ", ")
			for _, pull := range pulls {
				pullWords := strings.Split(pull, " ")
				count, err := strconv.Atoi(pullWords[0])
				if err != nil {
					fmt.Printf("%w", err)
					return
				}
		...
		if !isImpossible {
			possibleIndexes = append(possibleIndexes, gameIndex)
		}
	}
	...
	fmt.Printf("indexes %d sum %d", possibleIndexes, sum)
}
